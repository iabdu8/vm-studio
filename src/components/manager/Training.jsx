...
export function Training({ company, profile }) {
  ...
  const [selected,  setSelected]    = useState(null); // training detail view
  ...
  const [picked,   setPicked]   = useState([]); // selected staff ids
  ...
  const stats = (t) => {
    ...
    const scores  = att.filter(a => a.score != null).map(a => a.score);
                  ...
                  <button key={st} onClick={() => updateAttendee(a.id, "status", st)}
                    style={{
                      ...
                      background: a.status===st
                        ...
                        : "transparent",
                      color: a.status===st
                        ? st==="present" ? "#0a0a0f" : st==="absent" ? "#fff" : C.mutedColor
                        : C.mutedColor,
                    }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:10 }}>
              <div>
                ...
                <input style={{ ...S.inp, marginBottom:0, textAlign:"center", fontSize:16, fontWeight:700 }}
                  ...
                  value={a.score ?? ""}
                  onChange={e => updateAttendee(a.id, "score", e.target.value ? Number(e.target.value) : null)}/>
              </div>
              <div>
    ...
    <div>
      ...
      {showForm && (
        <div style={S.card}>
          ...
          {/* Staff picker */}
          <div style={S.h3}>Add Attendees</div>
          <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:14 }}>
            <button className="pill-btn" onClick={() =>
              setPicked(picked.length === staff.length ? [] : staff.map(s => s.id))}
          ...
          <div style={{ display:"flex", gap:8 }}>
            <button className="btnP" style={{ ...S.btnP, flex:1 }}
              onClick={createTraining} disabled={saving}>
      ...
      {trainings.map(t => {
        ...
        return (
          <div key={t.id} style={{ ...S.card, cursor:"pointer" }} onClick={() => setSelected(t)}>
            ...
            {s.total === 0 && (
              <div style={{ fontSize:12, color:C.mutedColor }}>No attendees added yet</div>
            )}
