...
import { getLocale, t as tr } from "../../lib/i18n.js";
...
export function TasksTable({ tasks, branches = [], showBranchColumn = false, canDelete = false, onDeleteTask }) {
  if (tasks.length === 0) {
    return <div style={{ ...S.muted, textAlign:"center", padding:30 }}>{tr("common.noTasks", "No tasks yet.")}</div>;
  }
  ...
  const headers = [
    tr("tasks.task", "Task"),
    ...(showBranchColumn ? [tr("common.branch", "Branch")] : []),
    tr("common.priority", "Priority"),
    tr("common.due", "Due"),
    tr("common.controller", "Controller"),
    tr("common.status", "Status"),
    ...(canDelete ? [tr("common.actions", "Actions")] : []),
  ];
        ...
        <table style={{ width:"100%", borderCollapse:"collapse", minWidth: showBranchColumn ? 720 : 680 }}>
          <thead>
            <tr>
              {headers.map((h, i, arr) => (
                <th key={h} style={{
                  padding:"14px 16px", textAlign:"start", fontSize:13, fontWeight:800,
            ...
            {sorted.map((t, i) => {
              ...
              return (
                <tr key={t.id}>
                  ...
                  <td style={{ ...cellStyle, fontSize:14, color:C.mutedColor, fontWeight:600, whiteSpace:"nowrap" }}>
                    {t.due_date
                      ? new Date(t.due_date).toLocaleDateString(getLocale(), { weekday:"short", day:"numeric", month:"short" })
                  ...
                  <td style={{ ...cellStyle, borderRight: canDelete ? cellStyle.borderRight : "none" }}>
                    <span style={S.chip((t.is_done||t.done)?"approved":"pending")}>
                      {(t.is_done||t.done) ? tr("common.done", "Done") : tr("common.open", "Open")}
                    </span>
                  </td>
                  {canDelete && (
