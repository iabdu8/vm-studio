...
// Today's tasks only, each one under the VM who actually did it (from their
// submission) — with before/after photos and a comment thread.
export function DailyTasksFeed({ tasks, submissions = [], branches = [], profile }) {
  const [lightbox, setLightbox] = useState(null);
  const [openId,   setOpenId]   = useState(null);

  const todayKey = new Date().toISOString().slice(0, 10);
  const todayTasks = tasks.filter(t => (t.due_date ?? (t.created_at ?? "").slice(0, 10)) === todayKey);

  if (todayTasks.length === 0) {
    return <div style={{ ...S.muted, textAlign:"center", padding:30 }}>No tasks scheduled for today.</div>;
  }
